...
const LoanSchema = z.object({
  ...
  loan_date:         z.string().date({ message: 'Podaj prawidłową datę' }),
  purpose:           z.string().min(5, 'Cel musi mieć co najmniej 5 znaków').max(500),
  loaned_to_name:    z.string().min(2, 'Podaj imię i nazwisko').max(200),
...
export async function POST(req: NextRequest) {
  ...
  // Pobierz następny numer wpisu (ta sama sekwencja co trip_entries)
  const { data: nextNum, error: seqError } = await supabase
    .schema('vat_km')
    .rpc('next_entry_number', { p_vehicle_id: d.vehicle_id })
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      purpose:           d.purpose,
      loaned_to_name:    d.loaned_to_name,
      loaned_to_user_id: d.loaned_to_user_id || null,
      odometer_at_issue: d.odometer_at_issue,
      notes:             d.notes || null,
      created_by:        user.id,
    })
    .select()
