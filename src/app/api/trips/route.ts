...
export async function POST(req: NextRequest) {
  ...
  // Pobierz następny numer wpisu (atomowa sekwencja)
  const { data: nextNum, error: seqError } = await supabase
    .schema('vat_km')
    .rpc('next_entry_number', { p_vehicle_id: data.vehicle_id })

  if (seqError) {
    return NextResponse.json({ error: seqError.message }, { status: 400 })
  }

  // Zapisz wpis — trigger validate_odometer_continuity zadba o spójność licznika
  ...
  if (insertError) {
    // Trigger bazy danych zwróci czytelny komunikat o błędzie licznika
